...
export const ReportHeader: React.FC<ReportHeaderProps> = ({
  ...
  return (
    <div className="hidden print:block w-full border-b-2 border-text-main pb-2 mb-1">
      {" "}
      <div className="flex justify-between items-center w-full mb-2">
        ...
        <div className="text-right flex flex-col gap-2 max-w-[75%]">
          ...
          <div className="space-y-1.5">
            {" "}
            <p className="text-[14px] font-[800] text-text-main font-cairo">
              {activeCompany.address}
            </p>{" "}
            {activeCompany.phones && (
              <p
                className="text-[14px] font-[800] text-text-main font-cairo"
                dir="rtl"
              >
                {" "}
                <span className="ml-1">هاتف:</span>{" "}
                <span dir="ltr" className="inline-block tracking-wider">
                  {activeCompany.phones}
                </span>{" "}
              </p>
            )}{" "}
      ...
      <div className="flex justify-center my-1">
        {" "}
        <div className="bg-text-main text-white px-12 py-1 rounded-md">
          {" "}
      ...
      <div className="flex justify-between items-center text-[11px] border-t border-border-main pt-2 mt-1">
        ...
        {dynamicData.map((item, index) => (
          <div key={index} className="flex items-center gap-1">
            {" "}
            <span className="font-[800] text-text-main whitespace-nowrap">
              {item.label}:
            </span>{" "}
            <span className="font-[900] text-text-main border-b border-text-muted border-dotted px-4 min-w-[80px] text-center">
              {" "}
              {item.value}{" "}
            </span>{" "}
          </div>
