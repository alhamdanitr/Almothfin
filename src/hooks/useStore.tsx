...
export function StoreProvider({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    const advancesRef = collection(db, 'companies', activeCompanyId, 'advances');
    ...
    const unsubscribeAdvances = onSnapshot(advancesRef, (snapshot) => {
      const advancesData = snapshot.docs.map(doc => ({ id: doc.id, ...doc.data() } as Advance));
  ...
  const addAdvance = async (advance: Omit<Advance, 'id'>) => {
    if (!activeCompanyId) return;
    try { await setDoc(doc(collection(db, 'companies', activeCompanyId, 'advances')), advance); } catch(e) { console.error(e); }
  };

  const updateAdvance = async (id: string, updated: Partial<Advance>) => {
    if (!activeCompanyId) return;
    try { await updateDoc(doc(db, 'companies', activeCompanyId, 'advances', id), updated); } catch(e) { console.error(e); }
  };

  const deleteAdvance = async (id: string) => {
    if (!activeCompanyId) return;
    try { await deleteDoc(doc(db, 'companies', activeCompanyId, 'advances', id)); } catch(e) { console.error(e); }
  };

  return (
    <StoreContext.Provider value={{
      companies, activeCompanyId, activeCompany, addCompany, updateCompany, switchCompany, deleteCompany,
      workers, records, advances, addWorker, updateWorker, deleteWorker, addRecord, addBulkRecords, deleteRecord, updateRecord,
