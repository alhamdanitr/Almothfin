...
export default function BulkEntry() {
  ...
  const handleEntryChange = (date: string, field: string, value: string) => {
    setEntries(prev => ({
      ...prev,
      [date]: {
        ...(prev[date] || { attendance: 'full', allowance: '', advancePayment: '', delayMinutes: '', note: '' }),
        [field]: value
      }
    }));
  };
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        <div>
          <h2 className="text-2xl font-bold text-text-main dark:text-white">الترحيل الجماعي الفردي</h2>
          <p className="text-sm text-text-muted mt-1">تعبئة الحضور والصرفيات لعامل محدد عبر فترة زمنية</p>
          {hasExistingRecords && (
        ...
        <div className="flex flex-col sm:flex-row items-center gap-3 w-full md:w-auto">
          <div className="relative w-full sm:w-48">
            ...
            <select
              ...
              className="w-full pl-3 pr-10 py-2 text-sm bg-surface dark:bg-slate-800 border border-border-main dark:border-slate-700 rounded-xl focus:ring-2 focus:ring-primary outline-none text-text-main dark:text-white shadow-sm appearance-none"
            >
          ...
          <div className="flex items-center space-x-2 space-x-reverse w-full sm:w-auto">
            <div className="relative w-full">
              <input 
                ...
                className="w-full px-3 py-2 text-sm bg-surface dark:bg-slate-800 border border-border-main dark:border-slate-700 rounded-xl focus:ring-2 focus:ring-primary outline-none text-text-main dark:text-white shadow-sm"
              />
            </div>
      ...
      {!selectedWorkerId ? (
        <div className="p-8 text-center text-text-muted bg-surface dark:bg-slate-800 rounded-2xl border border-border-main dark:border-slate-700">
          يرجى اختيار العامل وتحديد الفترة الزمنية
        </div>
      ) : datesInRange.length === 0 ? (
        <div className="p-8 text-center text-danger bg-red-50 dark:bg-red-900/20 rounded-2xl">
        ...
        <div className="bg-surface dark:bg-slate-800 shadow-sm rounded-2xl border border-border-main dark:border-slate-700 overflow-hidden">
          <div className="bg-brand-bg dark:bg-slate-900/50 px-6 py-4 border-b border-border-main dark:border-slate-700 flex justify-between items-center">
            <h3 className="font-bold text-text-main dark:text-white flex items-center">
          ...
          <div className="hidden lg:block overflow-x-auto">
            <table className="min-w-full text-right divide-y divide-gray-200 dark:divide-slate-700">
              <thead className="bg-brand-bg dark:bg-slate-900/50">
                <tr>
                  <th scope="col" className="px-4 py-3 text-sm font-semibold text-text-main dark:text-gray-300 w-[15%]">التاريخ</th>
              ...
              <tbody className="divide-y divide-gray-100 dark:divide-slate-700/50">
                {datesInRange.map((date) => {
                  const entry = entries[date] || { attendance: 'full', allowance: '', advancePayment: '', delayMinutes: '', note: '' };
                  
                  return (
                    <tr key={date} className="hover:bg-brand-bg dark:hover:bg-slate-700/50 transition-colors">
                      <td className="px-4 py-3 whitespace-nowrap text-sm font-medium text-text-main dark:text-white" dir="ltr">
                        {date}
                      </td>
                      ...
                      <td className="px-4 py-3 whitespace-nowrap">
                        <input
                          ...
                          onChange={(e) => handleEntryChange(date, 'delayMinutes', e.target.value)}
                          className="w-full px-3 py-2 bg-surface dark:bg-slate-900 border border-border-main dark:border-slate-600 rounded-lg focus:ring-2 focus:ring-primary outline-none text-text-main dark:text-white transition-colors"
                        />
                      </td>
                      <td className="px-4 py-3 whitespace-nowrap">
          ...
          <div className="lg:hidden divide-y divide-gray-100 dark:divide-slate-700/50">
            {datesInRange.map((date) => {
              const entry = entries[date] || { attendance: 'full', allowance: '', advancePayment: '', delayMinutes: '', note: '' };
              return (
                <div key={date} className="p-4 space-y-4 hover:bg-brand-bg dark:hover:bg-slate-700/50 transition-colors">
                  <div className="font-bold text-lg text-text-main dark:text-white border-b border-border-main dark:border-slate-700 pb-2" dir="ltr">
                    {date}
                  </div>
