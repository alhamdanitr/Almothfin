...
async function run() {
  console.log("Wiping all current records...");
  const oldSnap = await getDocs(collection(db, 'records'));
  const allDocs = oldSnap.docs;
  ...
  console.log("Wiped", allDocs.length, "old records.");
  
  console.log("Fetching exact same request as in frontend...");
  ...
  if (!data.records && !data.allEntries) {
    console.log("No records found", data);
    return;
  }
  
  const recordsData = data.records || data.allEntries;
  
  console.log("Got records from GAS:", recordsData.length);
  
  const workersSnap = await getDocs(collection(db, 'workers'));
  const workers = workersSnap.docs.map(d => ({ id: d.id, ...d.data() }));
  
  const mappedRecords = recordsData.map((rec) => {
